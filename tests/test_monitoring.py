...
def _call_forecast_for_monitoring() -> None:
    """Trigger a forecast request so monitoring metrics are emitted."""
    params = {
        "id_well": "POZO-001",
        "date_start": "2026-04-02",
        "date_end": "2026-04-04",
    }
    client.get("/api/v1/forecast", params=params, headers=VALID_HEADERS)
...
def test_forecast_request_is_recorded_in_metrics() -> None:
    ...
    _call_forecast_for_monitoring()
    response = client.get("/metrics")

    assert response.status_code == 200
    assert (
        'forecast_api_requests_total{method="GET",path="/api/v1/forecast",'
        'status_code="200"}' in response.text
    )
    assert (
        "forecast_api_request_duration_seconds_count"
        '{method="GET",path="/api/v1/forecast",status_code="200"}' in response.text
    )
